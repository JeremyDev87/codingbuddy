...
describe('esmImport path guard', () => {
  it('should reject relative paths', async () => {
    await expect(esmImport('./foo.mjs')).rejects.toThrow(
  ...
  it('should reject paths outside project root via ../ traversal', async () => {
    ...
    await expect(esmImport(outsidePath)).rejects.toThrow(
      'path must be within project root',
    );
  });

  it('should reject bare module specifiers', async () => {
    await expect(esmImport('fs')).rejects.toThrow(
      'only absolute paths are accepted',
    );
  });
  ...
  it('should reject parent directory of project root', async () => {
    ...
    await expect(esmImport(parentDir)).rejects.toThrow(
      'path must be within project root',
    );
  });
  ...
  it('should accept paths within project root (file may not exist)', async () => {
    ...
    await expect(esmImport(validPath)).rejects.not.toThrow(
      'path must be within project root',
    );
  });
});
